...
// File: contracts/helpers/interfaces/IKLIMAERC20.sol



pragma solidity ^0.8.4;

interface IKLIMAERC20 {
...
// File: contracts/regular/KlimaTreasury.sol
...
contract KlimaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SKLIMA }
    ...
    uint public sKLIMAQueue; // Delays change to sKLIMA address
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IERC20Mintable( KLIMA ).mint( msg.sender, send_ );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sKLIMA ).balanceOf( msg.sender ); // Can only borrow against sKLIMA held
    ...
    function repayDebtWithKLIMA( uint _amount ) external {
        ...
        emit RepayDebt( msg.sender, KLIMA, _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( KLIMA ).totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match KLIMA decimals
            value_ = _amount.mul( 10 ** IERC20( KLIMA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SKLIMA ) { // 9
            sKLIMAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
